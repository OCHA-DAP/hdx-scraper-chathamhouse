...
class ChathamHouseModel:
    ...
    @staticmethod
    def get_co2(scaled_number_hh, multiplier, values, baseline_target, comtype):
        return scaled_number_hh / ChathamHouseModel.co2_divisor * multiplier * \
               values['CO2 %s Type %s' % (baseline_target, comtype)]

    def get_kWh_per_hh_per_yr(self, electiers, elecappliances):
    ...
    def calculate_ongrid_lighting(self, hh_grid_access, electiers, elecappliances, elecco2):
        kWh_per_hh_per_yr = self.get_kWh_per_hh_per_yr(electiers, elecappliances)
    ...
    def calculate_offgrid_lighting(self, tier, hh_offgrid, noncamplightingoffgridtype,
                                   lightingoffgridcost, multiplier=1.0):
        ...
        capital_costs = self.get_capital(scaled_number_hh, lightingoffgridcost, baseline_target,
                                         noncamplightingoffgridtype)
        co2_emissions = self.get_co2(scaled_number_hh, multiplier, lightingoffgridcost, baseline_target,
                                     noncamplightingoffgridtype)

        return expenditure, capital_costs, co2_emissions

    def calculate_noncamp_offgrid_lighting(self, pop_type, tier, iso3, hh_offgrid, noncamplightingoffgridtypes,
                                           lightingoffgridcost, elecco2):
        noncamplightingoffgridtype = noncamplightingoffgridtypes['%s %s Type' % (pop_type, tier)]
        multiplier = elecco2[iso3]
        return self.calculate_offgrid_lighting(tier, hh_offgrid, noncamplightingoffgridtype, lightingoffgridcost,
                                               multiplier)

    def calculate_camp_offgrid_lighting(self, tier, hh_offgrid, camptypes, lightingoffgridcost):
    ...
    def calculate_solid_cooking(self, tier, hh_no_nonsolid_access, noncampcookingsolidtype, cookingsolidcost):
        scaled_number_hh = hh_no_nonsolid_access * self.constants['Cooking Solid Scaling Factor']

        baseline_target = self.get_baseline_target(tier)
        expenditure = self.get_expenditure(scaled_number_hh, cookingsolidcost, baseline_target, noncampcookingsolidtype)
        capital_costs = self.get_capital(scaled_number_hh, cookingsolidcost, baseline_target, noncampcookingsolidtype)
        co2_emissions = self.get_co2(scaled_number_hh, 12.0, cookingsolidcost, baseline_target, noncampcookingsolidtype)
